...
int Confirm_Fault_Pair(HASH hash, DICT* dic) {
	...
	int test_number, hash_number, fault_number, search_number,i,x;																	//ループ変数
...
int* Subdivide_Judge(HASH hash, DICT sim_dic, int hash_number) {
	...
	devide_hash_number = (int*)malloc(sizeof(int) * n_unconf_fault);
	delete_hash_number = (int*)malloc(sizeof(int) * n_unconf_fault);
